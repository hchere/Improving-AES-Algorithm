...
// S-box four bytes
// Each of the bytes passed in key expansion core is replaced by S-box
in[0] = s_box[in[0]]; in[1] = s_box[in[1]];
in[2] = s_box[in[2]]; in[3] = s_box[in[3]];

// Rcon
// i value will be first 10 or 11
in[0] ^=rcon[i];

}

//The AES key expansion algorithm takes as input a four-word (16-byte) key and 
//produces a linear array of 44 words (176 bytes). 176 Bytes(original key + 10th round)
...
{   
    ...
    //XOR temp with [bytesGenerateed -16], and store in expandedKeys:
...
void SubBytes(unsigned char *state)
{
    ...
    // for every byte in state, we'll replace it with s_box value
    //  each byte in a state becomes whatever the index of byte is in the s_box
...
{   
    ...
    // circular shift(left)
    // We have to use this method as we 
    // can't use bitwise shift operator! (Atleast what I think and know)
    // Rotate left:
    // unsigned int *q = (unsigned int *)in;
    // *q = (*q >> 8) | ((*q & 0xff) <<24);           // left shift operator "<<" and Right shift operator ">>" 
    // works for 1 shift. What about 2, 3?
   ...
   // now copy the matrix into temporary matrix
...
void MixColumns(unsigned char *state) {
 // temp as you do not want to change the original
...
{
    // copy message to seperate array called state
    ...
    // key will be different for each of these AddRoundKey calls but for now, we just use a single key
